...
ssh_sites_result.each do |n|
  if node[:jenkins][:server][:enabled_remote_hosts].include? n
    ...
    job_name3 = "remote-SSH-chef-client-#{n}"
    ...
    node[:jenkins][:server][:enabled_remote_hosts] << n
...
job_name_rws = "remote-SSH-chef-client-all-web-servers"
...
jenkins_ssh_nodes.each do |n|
  if (n.run_list.run_list_items.include?("role[maintenance-page-check-load]") ||
	...
	# automatic job
    job_name3 = "remote-SSH-chef-client-maintenance-page"
    job_config3 = File.join(node[:jenkins][:server][:home], "#{job_name3}-config.xml")
	...
	# switch on job
    job_name3 = "remote-SSH-chef-client-maintenance-page-switch-on"
    job_config3 = File.join(node[:jenkins][:server][:home], "#{job_name3}-config.xml")
	...
	# switch off job
    job_name3 = "remote-SSH-chef-client-maintenance-page-switch-off"
    job_config3 = File.join(node[:jenkins][:server][:home], "#{job_name3}-config.xml")
